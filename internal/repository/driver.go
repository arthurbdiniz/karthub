...
func (r *DriverRepository) List(ctx context.Context) ([]models.Driver, error) {
	rows, err := r.db.QueryContext(ctx,
		"SELECT id, user_id, name, nickname, avatar, bio, joined_at, created_at, updated_at FROM drivers ORDER BY name")
	if err != nil {
	...
	for rows.Next() {
		var d models.Driver
		if err := rows.Scan(&d.ID, &d.UserID, &d.Name, &d.Nickname, &d.Avatar, &d.Bio, &d.JoinedAt, &d.CreatedAt, &d.UpdatedAt); err != nil {
...
func (r *DriverRepository) GetByID(ctx context.Context, id int64) (*models.Driver, error) {
	...
	err := r.db.QueryRowContext(ctx,
		"SELECT id, user_id, name, nickname, avatar, bio, joined_at, created_at, updated_at FROM drivers WHERE id = ?", id,
	).Scan(&d.ID, &d.UserID, &d.Name, &d.Nickname, &d.Avatar, &d.Bio, &d.JoinedAt, &d.CreatedAt, &d.UpdatedAt)
...
func (r *DriverRepository) Create(ctx context.Context, d *models.Driver) error {
	result, err := r.db.ExecContext(ctx,
		"INSERT INTO drivers (user_id, name, nickname, avatar, bio, joined_at) VALUES (?, ?, ?, ?, ?, ?)",
		d.UserID, d.Name, d.Nickname, d.Avatar, d.Bio, d.JoinedAt,
	)
	if err != nil {
...
func (r *DriverRepository) Update(ctx context.Context, d *models.Driver) error {
	_, err := r.db.ExecContext(ctx,
		"UPDATE drivers SET name = ?, avatar = ?, bio = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
		d.Name, d.Avatar, d.Bio, d.ID,
	)
	if err != nil {
...
func (r *DriverRepository) GetByUserID(ctx context.Context, userID int64) (*models.Driver, error) {
	var d models.Driver
	err := r.db.QueryRowContext(ctx,
		"SELECT id, user_id, name, nickname, avatar, bio, joined_at, created_at, updated_at FROM drivers WHERE user_id = ?", userID,
	).Scan(&d.ID, &d.UserID, &d.Name, &d.Nickname, &d.Avatar, &d.Bio, &d.JoinedAt, &d.CreatedAt, &d.UpdatedAt)
	if err == sql.ErrNoRows {
		return nil, nil
	}
